...
# Make plots
df_results$Method <- factor(df_results$Method, levels = c("CompNMF", "CompNMFcos","SignatureAnalyzer" , "BayesNMF"))
...
pK <- df_results %>%
  ...
  scale_color_manual(values = c("forestgreen", "blue", "firebrick", "darkorange"))+
  scale_fill_manual(values = c("forestgreen", "blue", "firebrick", "darkorange"))+
  ylim(c(3, 20))+
  theme_bw()+
...
ggsave(filename = "figures/Sparse_indels_simulation.pdf", height = 5.72, width = 5.59 * 2)
