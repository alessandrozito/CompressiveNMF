...
# resBayesNMF <- readRDS("output/main_simulation/Scenario_50_overdisp_0_Knew_2_theta_100/BayesNMF_brouwer/results_1.rds.gzip")
# data_all <- readRDS("output/main_simulation/Scenario_50_overdisp_0_Knew_2_theta_100/data.rds.gzip")
# data <- data_all[[1]]
# sig <- resBayesNMF$Signatures
# rownames(sig) <- rownames(CompressiveNMF::COSMIC_v3.4_SBS96_GRCh37)
# CompressiveNMF:::plot.SBS.signature(sig)
# tt <- resBayesNMF$Theta
# colnames(tt) <- 1:ncol(tt) 
# CompressiveNMF:::plot_weights(Wmat = tt)
# 
# plot(resBayesNMF$Theta)
# 
...
# select <- apply(sigs_norm, 2, function(x) cosine(x, rep(1, 96))) < 0.975
# 
# plot(sigs_norm[, select] %*% Theta_norm[select,], sigs_norm[, ] %*% Theta_norm[,])
# 
# sigs_norm <- apply(Rmean, 2, function(x) x/sum(x))
# rownames(sigs_norm) <- rownames(CompressiveNMF::COSMIC_v3.4_SBS96_GRCh37)
# CompressiveNMF:::plot.SBS.signature(sigs_norm[, select])
# CompressiveNMF:::plot.SBS.signature(sigs_norm[, 1:10])
# 
# 
# 
# plot(colMeans(Theta_mean), apply(sigs_norm, 2, function(x) cosine(x, rep(1, 96))))
# plot(colMeans(Lambda), apply(sigs_norm, 2, function(x) cosine(x, rep(1, 96))))
# 
# 
# res <- readRDS("output/main_simulation/Scenario_100_overdisp_0_Knew_2_theta_100/CompressiveNMF.rds.gzip")
# 
# res
# 
# sigs <- read.table("output/main_simulation/Scenario_50_overdisp_0_Knew_2_theta_100/BayesNMF_brouwer/output/sim_1/Signatures.txt")
# 
# dim(sigs)
# 
# 
# 
# 
# lambda <- read.table("output/main_simulation/Scenario_50_overdisp_0_Knew_2_theta_100/BayesNMF_brouwer/output/sim_1/all_lambda.txt")
# sort(c(lambda$V1))
# 
# plot(Lambda[, 1], type = "l", ylim = c(0, max(Lambda)))
# for(i in 2:ncol(Lambda)){
#   lines(Lambda[, i], col = select[i])
# }
# 
# dim(lambda)
# 
# directory <- "output/main_simulation/Scenario_50_overdisp_0_Knew_2_theta_100/BayesNMF_brouwer/output/sim_1/Signatures.txt.gz"
# sigs <- as.matrix(read_table(directory, show_col_types = FALSE))
# 
# 
directory <- "output/main_simulation/Scenario_50_overdisp_0_Knew_2_theta_100/BayesNMF_brouwer/output/sim_1/"

R <- as.matrix(read_table(paste0(directory, "Signatures.txt"),
                          col_names = FALSE, show_col_types = FALSE))

R_all <- array(NA, dim = c(nrow(R)/96, 96, 20))
n <- nrow(R)
id_all <- seq(1, n, by = 96)
for(s in 1:(n/96 - 1)) R_all[s, ,] <- R[id_all[s]:(id_all[s+1] - 1), ]
R_all[n/96, ,] <- R[id_all[s + 1]:n, ]
Rmean <- apply(R_all, c(2,3), mean)


Rtrue <- as.matrix(read_table(paste0(directory, "Signatures_res.txt"),
                          col_names = FALSE, show_col_types = FALSE))

rownames(Rtrue) <- rownames(CompressiveNMF::COSMIC_v3.4_SBS96_GRCh37)
CompressiveNMF:::plot.SBS.signature(Rtrue[, 1:20])

Thetatrue <-  as.matrix(read_table(paste0(directory, "Loadings_res.txt"),
                                           col_names = FALSE, show_col_types = FALSE))
norm_weight <- colSums(Rtrue)
sigs_norm <- sapply(1:20, function(i) Rtrue[, i]/norm_weight[i])
Theta_norm <- t(sapply(1:20, function(i) Thetatrue[, i]*norm_weight[i]))

a1 <- rowMeans(apply(Theta_norm, 2, function(x) x/sum(x)))

a2 <- apply(sigs_norm, 2, function(x) cosine(x, rep(1, 96)))
plot(a1, a2)
...
# Theta <- as.matrix(read_table(paste0(directory, "Loadings.txt"), 
#                               show_col_types = FALSE, col_names = FALSE))
# 
# Theta_all <- reshape_python_array(Theta, dimension = 50)
# Thetamean <- apply(Theta_all, c(2,3), mean)
# Thetatrue <-  as.matrix(read_table(paste0(directory, "Loadings_res.txt"), 
#                                            col_names = FALSE, show_col_types = FALSE))
# 
# 
